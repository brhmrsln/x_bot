...
class TradingEngine:
    def __init__(self, client: BinanceFuturesClient, strategy: Strategy):
        ...
        self.leverage = int(getattr(settings, 'LEVERAGE', 10)) # Get leverage from settings

        # In-memory state management for open positions
        self.open_positions = {} 
        ...
        logger.info(f"  Leverage: {self.leverage}x") # Log a ekleyelim
        logger.info(f"  Stop-Loss Percentage: {self.sl_percentage*100}%")
        logger.info(f"  Take-Profit Percentage: {self.tp_percentage*100}%")
    ...
    def _handle_trade_closure(self, symbol, entry_trade_info, closing_order, exit_reason):
        """Calculates PnL, gets exit commission, and logs the completed trade to the CSV file."""
        ...
        # Get exit commission from the closing order's trades
        exit_commission = 0.0
        ...
        pnl_percentage = (pnl_usdt / entry_nominal_value) if entry_nominal_value != 0 else 0.0
        
        logger.info(f"Trade Closed for {symbol}: Gross PnL={pnl_usdt:.4f} USDT, "
                    f"Total Commission={total_commission:.6f}, Net PnL={net_pnl_usdt:.4f} USDT")
